...
export interface IDailyOrder extends Document {
  ...
  meals: {
    ...
    status: "scheduled" | "prepared" | "dispatched"|"partially_refunded"|"cancelled"|"partially_delivered" |"confirmed"| "out_for_delivery"|"delivered" | "delayed" |"pending"|"preparing"|"prepared";
    statusHistory?: { status: "scheduled" | "prepared" | "dispatched" | "delivered" | "delayed" |"preparing";  updatedAt: Date }[];
    locked: boolean;
    price?: number;
    costPrice?: number;
    quantity?: number;
  ...
  orderStatus:
    ...
    | "partially_delivered" 
    | "delayed"
    | "partially_refunded" |"dispatched";
  paymentStatus: "pending" | "paid" | "failed" | "refunded" | "partial_refund" |"dispatched";
  paymentMethod: "card" | "cash" | "wallet" | "stripe" | "cod";
  updatedAt:string;
  currency: string;
 
}

const DailyOrderSchema = new Schema<IDailyOrder>(
  {
...
DailyOrderSchema.pre("save", function (next) {
  if (this.meals && this.meals.length > 0) {
    ...
    if (deliveredMeals === totalMeals) this.orderStatus = "delivered";
    else if (delayedMeals === totalMeals) this.orderStatus = "delayed";
    else if (deliveredMeals > 0 && delayedMeals > 0) this.orderStatus = "partially_delivered";
    else if (this.meals.some((m) => m.status === "dispatched")) this.orderStatus = "out_for_delivery";
    else if (this.meals.some((m) => m.status === "prepared")) this.orderStatus = "preparing";
    else this.orderStatus = "confirmed";
  }
  next();
});
...
DailyOrderSchema.pre("save", function (next) {
  if (this.meals && this.meals.length > 0) {
    // Calculate totals
    this.totalPrice = this.meals.reduce(
      (sum: number, meal) => sum + (meal.price || 0) * (meal.quantity || 1),
      0
    );
    this.totalCost = this.meals.reduce(
      (sum: number, meal) => sum + (meal.costPrice || 0) * (meal.quantity || 1),
      0
    );
    this.profit = this.totalPrice - this.totalCost;
    ...
    // Determine order status
    const totalMeals = this.meals.length;
    const deliveredMeals = this.meals.filter((m: IDailyOrder["meals"][0]) => m.status === "delivered").length;
    const delayedMeals = this.meals.filter((m: IDailyOrder["meals"][0]) => m.status === "delayed").length;

    if (deliveredMeals === totalMeals) this.orderStatus = "delivered";
    else if (delayedMeals === totalMeals) this.orderStatus = "delayed";
    else if (deliveredMeals > 0 && delayedMeals > 0) this.orderStatus = "partially_delivered";
    else if (this.meals.some((m: IDailyOrder["meals"][0]) => m.status === "dispatched")) this.orderStatus = "out_for_delivery";
    else if (this.meals.some((m: IDailyOrder["meals"][0]) => m.status === "prepared")) this.orderStatus = "preparing";
    else this.orderStatus = "confirmed";
  }
  next();
});
...
DailyOrderSchema.methods.getRevenueByStatus = function () {
  ...
  this.meals.forEach((meal: IDailyOrder["meals"][0]) => {
...
export const DailyOrder = model<IDailyOrder>("DailyOrder", DailyOrderSchema);
